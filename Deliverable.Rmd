...
## Films with Monthly Highest Gross Box Office 

```{r}
kable(h_2 %>% dplyr::select(-genres) %>% distinct() %>% dplyr::select(title, month) %>% arrange(month), "html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE, position = "float_right")
```
...
```{r}
kable(l_1 %>% dplyr::select(-genres) %>% distinct() %>% dplyr::select(title, year), "html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE, position = "float_right")
```
...
## Lowest Monthly Gross Box Office 

```{r}
kable(l_2 %>% dplyr::select(-genres) %>% distinct() %>% dplyr::select(title, month) %>% arrange(month), "html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE, position = "float_right")
```
